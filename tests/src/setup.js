...
before(async function() {
  this.config = config;
  this.database = await createDatabase({ config });
  this.email = await createEmail({ config });
  this.models = {};
  this.models.Migration = await createMigration({ config, database: this.database });
  this.models.Credential = await createCredential({ config, database: this.database });
  this.models.User = await createUser({ config, database: this.database, models: this.models, errors });
