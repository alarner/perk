...
if(!config.webserver.hostname) {
...
else if(config.webserver.https && ((!config.webserver.https.port && !config.webserver.https.proxyPort) || !config.webserver.https.keyPath || !config.webserver.https.certPath)) {
	logger.error('When using https you must provide a https.port or https.proxyPort, https.keyPath and https.certPath in config/webserver.js');
	process.exit(1);
}
else {
	let port = 3000;
	if(config.webserver.https) {
		port = config.webserver.https.proxyPort || config.webserver.https.port;
	...
	else if(config.webserver.http && (config.webserver.http.proxyPort || config.webserver.http.port)) {
		port = config.webserver.http.proxyPort || config.webserver.http.port;
	}
