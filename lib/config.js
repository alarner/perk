...
if(!config.webserver.hostname) {
	logger.warn('A hostname must be specified in config/webserver.js');
}
else if(config.webserver.ssl && (!config.webserver.ssl.port || !config.webserver.ssl.keyPath || !config.webserver.ssl.certPath)) {
	logger.warn('When using https you must provide a ssl.port, ssl.keyPath and ssl.certPath in config/webserver.js');
}
else {
